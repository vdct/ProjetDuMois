...
WITH minc AS (
	SELECT DISTINCT ON (project_id, boundary) project_id, boundary, amount
	FROM pdm_feature_counts_per_boundary
	ORDER BY project_id, boundary, ts
),
maxc AS (
	SELECT DISTINCT ON (project_id, boundary) project_id, boundary, amount
	FROM pdm_feature_counts_per_boundary
	ORDER BY project_id, boundary, ts DESC
),
stats AS (
	SELECT project_id, boundary, json_object(array_agg(ts::date::text), array_agg(amount::text)) AS stats
	FROM pdm_feature_counts_per_boundary
	GROUP BY project_id, boundary
)
SELECT s.project_id, s.boundary, b.id, b.name, b.admin_level, s.stats, maxc.amount - minc.amount AS nb, b.centre AS geom
FROM stats s
JOIN minc ON minc.project_id = s.project_id AND minc.boundary = s.boundary
JOIN maxc ON maxc.project_id = s.project_id AND maxc.boundary = s.boundary
...
CREATE OR REPLACE FUNCTION pdm_boundary_project_tiles(z integer, x integer, y integer, project_id int) RETURNS bytea AS $$
DECLARE
	result bytea;
BEGIN
	WITH bounds AS (
