...
-- Main count
INSERT INTO pdm_feature_counts (project_id, ts, amount) 
    SELECT :project_id as project_id, d.ts, count(fc.osmid) as amount
...
-- Boundary count
INSERT INTO pdm_feature_counts_per_boundary (project_id, boundary, ts, amount) 
    SELECT :project_id as project_id, fb.boundary, d.ts, count(fc.osmid) as amount
    FROM :changes_table fc
