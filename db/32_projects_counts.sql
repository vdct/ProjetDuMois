...
INSERT INTO pdm_feature_counts_per_boundary (project_id, boundary, ts, label, amount, len)
    SELECT :project_id as project_id, fb.boundary, d.ts, fl.label, count(fc.osmid) as amount, sum(fc.geom_len) as len
    FROM :changes_table fc
    ...
    LEFT JOIN :labels_table fl ON fl.osmid=fc.osmid AND fl.version=fc.version
    WHERE fc.action!='delete' AND fc.tagsfilter=true
    GROUP BY fb.boundary, d.ts, ROLLUP(fl.label)
