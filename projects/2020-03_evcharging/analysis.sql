...
SELECT current_setting('my.pdm.project_id') AS project, userid, ts, 'add_ev' AS contribution
FROM (
	SELECT *
	FROM osm_changes
...
INSERT INTO user_contributions
SELECT current_setting('my.pdm.project_id') AS project, userid, ts, 'edit_ev' AS contribution
FROM (
	SELECT *
	FROM osm_changes
	WHERE version != 1 AND ts_in_project(ts) AND tags->>'amenity'='charging_station'
...
-- Meta
INSERT INTO user_badges(userid, project, badge)
SELECT DISTINCT userid, 'meta', 'evcharging' AS badge
FROM user_contributions
WHERE project = current_setting('my.pdm.project_id')
ON CONFLICT DO NOTHING;
