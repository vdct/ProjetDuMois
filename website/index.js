...
function promises_featureCounts (project, boundary){
  ...
  if (project.statistics.count) {
    let boundaryWhere = " AND boundary IS NULL";
    if (boundary != null){
  ...
  if (p.statistics.count) {
    allPromises.push(
      pool
        ...
        WHERE project_id = $1 AND label IS NULL AND team IS NULL
        ORDER BY ts ASC
        `,
        [p.id]
        )
...
// Feature counts for a team
app.get("/projects/:name/counts/:team", (req, res) => {
  if (CONFIG.MAINTENANCE_MODE === true) {
    return res.redirect("/");
  }
...
app.get("/projects/:name/mappers/:team", (req, res) => {
  if (CONFIG.MAINTENANCE_MODE === true) {
    return res.redirect("/");
  }
